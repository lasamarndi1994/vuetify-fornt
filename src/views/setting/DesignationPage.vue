<template>
    ...
    <v-row>
        <v-data-table :headers="headers" :search="search" :items="designations" item-value="name"
                                        ...
                                        <v-row>
                                            <v-col cols="12" md="12" sm="12">
                                                <v-text-field clearable v-model="editedItem.name" :rules="[required]"
                                                    variant="outlined" label="Designation name"></v-text-field>
                                            </v-col>
                                            <v-col cols="12" md="12" sm="12">
                                                <v-select clearable label="Select Department" :rules="[required]"
                                                    v-model="editedItem.department_id" :items="departments"
                                                    variant="outlined" item-title="name" item-value="id">
...
const onSubmit = () => {
    ...
    if (editedIndex > -1) {
        Object.assign(designations[editedIndex], editedItem)
    } else {
        designations.push(editedItem)
    }
    close()
}
...
const formTitle = computed(() => {
    ...
    return editedIndex.value === -1 ? 'New Designation' : 'Edit Designation'
});
watchEffect(async () => {
    const dialog = (val) => {
        val || close()
...
const editItem = (item) => {
    editedIndex.value = designations.indexOf(item)
...
const deleteItemConfirm = () => {
    designations.splice(editedIndex, 1)
    closeDelete()
}
